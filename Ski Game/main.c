...
#include "gfx\ttank.h"		// header file created by pcx2gba
#include "gfx\ski.h"		// ski menu
#include "gfx\menu1.h"		// software sprite menu (where credits is highlighted)
#include "gfx\bg.h"		// in game scrolling snow background 
...
void DoIntro(void)
{
	...
	for(x = 0; x < 256; x++)				// palette for intro screen
		paletteMem[x] = ttankPalette[x];	
	for(y = 0; y < 160; y++)
		for(x = 0; x < 120; x++)
			PlotPixel(x,y,ttankData[y*120+x]);//logoData contains the color values of your pict
	WaitForVblank();
	Flip();
	FadeIn(2);
	Sleep(115);
...
void DoGame(void)
{
	...
	for(loop = 0; loop < 256; loop++)
		paletteMem[loop] = bgPalette[loop];	// background palette
	...
	bgscroll = 1;	// how fast background scrolls
